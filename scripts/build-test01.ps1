param([switch]$RunTests, [switch]$Lint, [string]$TestKeyFile)
...
$buildRoot = Join-Path $toolRoot 'builds/seu-test01'
...
$arguments = @(':app:assembleTrial', '--no-daemon', '--max-workers=1', '-Dorg.gradle.jvmargs=-Xmx1200m -XX:MaxMetaspaceSize=512m -Dfile.encoding=UTF-8', '-Pkotlin.compiler.execution.strategy=in-process', '-Dorg.gradle.parallel=false', '-Pandroid.overridePathCheck=true', '--console=plain')
if ($RunTests) { $arguments += ':app:testTrialUnitTest' }
if ($Lint) { $arguments += ':app:lintTrial' }
...
try {
    ...
    $apkBytes = [IO.File]::ReadAllBytes((Join-Path $resolvedBuild 'app/build/outputs/apk/trial/app-trial.apk'))
    [IO.File]::WriteAllBytes((Join-Path $output 'test01.apk'), $apkBytes)
    Get-FileHash -LiteralPath (Join-Path $output 'test01.apk') -Algorithm SHA256
} finally { Pop-Location; $env:SEU_AI_TEST_KEY_FILE = $previousKeyFile }
